 ...
 * @apiNote 预案系统的接收抽象类
 */
public abstract class AbstractPlanReceiver {
    private static final Logger logger = LoggerFactory.getLogger(AbstractPlanReceiver.class);

    @RabbitListener(queues = "#{planAcceptProgrammeQueue.name}")
    @RabbitHandler
    private void receiveGridManInstruction(Message message){
        ...
        if (messageType.equals(ProjectsMessageTypes.PROGRAMME_TYPE)){
            resolveDirectProgramme(body,objectType,source);
        }else {
     ...
     */
    public abstract void resolveDirectProgramme(String jsonInstruction, String objectType, String source);
