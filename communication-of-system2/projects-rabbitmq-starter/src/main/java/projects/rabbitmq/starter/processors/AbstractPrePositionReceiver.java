...
/**
 * @apiNote 为部委前置系统封装接收指令信息的 抽象类
 */
public abstract class AbstractPrePositionReceiver {
    private static final Logger logger = LoggerFactory.getLogger(AbstractPrePositionReceiver.class);

    @RabbitListener(queues = "#{prePositionAcceptInstructionQueue.name}")
    @RabbitHandler
    private void receivePrePositionInstruction(Message message){
    ...
    /**
     * @apiNote 为部委前置系统封装 处理指挥发来指令的 方法回调
